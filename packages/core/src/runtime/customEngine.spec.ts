...
// a missing config would run stock codex on the user's OpenAI account,
// explicit env_key handling isolates custom credentials, and a blank model makes codex quietly use
...
describe("runtime/spawn: custom engine gates", () => {
  ...
  it("customProviderFlags declares an explicit credential even for keyless endpoints", () => {
  ...
  it("trims legacy model values before sending them to the endpoint", () => {
    expect(customProviderFlags(cfg({ model: "  llama3  " }))).toContain('model="llama3"');
  });
});
...
describe("account/customEngineAuth: saveCustomEngineConfig validation", () => {
  ...
  it("does not echo credentials from an invalid URL into a surfaced error", async () => {
